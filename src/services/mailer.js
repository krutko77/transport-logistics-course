  ...
  return {
    ...
    <div style="font-family:Arial,sans-serif;max-width:700px;margin:0 auto">
      <div style="background:#CC0000;padding:20px;text-align:center">
        <h1 style="color:#fff;margin:0;font-size:20px">Результат тестирования</h1>
        <p style="color:rgba(255,255,255,0.85);margin:6px 0 0;font-size:15px">Курс «Транспортная логистика»</p>
      </div>
...
export async function sendResultEmail(result) {
  ...
  if (process.env.RESEND_API_KEY) {
    ...
    await resend.emails.send({
      from: process.env.RESEND_FROM || 'Тест по логистике <onboarding@resend.dev>',
  ...
  await transporter.sendMail({
    from: `"Тест по логистике" <${process.env.SMTP_USER}>`,
    to: process.env.MANAGER_EMAIL,
    subject,
    html,
  });
